...
def display_search(result, format):
    ...
    if type == "person":
        t = loader.get_template("user/raw.%s" % format)
        ret += u"%s" % t.render(Context({"u": element}))
    elif type == "group":
        t = loader.get_template("group/raw.%s" % format)
        ret += u"%s" % t.render(Context({"g": element}))
    elif type == "note":
        t = loader.get_template("note/raw.%s" % format)
        ret += u"%s" % t.render(Context({"n": element}))
    elif type == "photo":
        t = loader.get_template("photo/raw.%s" % format)
